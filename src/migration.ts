...
export class MigrationService {
  ...
  private extractAgentMetadata(
    ...
    const modelIndex = pathComponents.findIndex(
      (part) => part.includes('claude') || part.includes('gpt') || part.includes('gemini')
    );
  ...
  async migrateEntries(
    ...
    const { batchSize = 50, onProgress, continueOnError = true } = options;
  ...
  private async migrateEntry(
    ...
    const exists = await (this.dbManager as any).checkEntryExists(
      filePath,
      timestamp.getTime(),
      content
    );
    ...
    const entryType =
      content.includes('## Feelings') || content.includes('## Project Notes')
        ? 'thoughts'
        : 'simple';
